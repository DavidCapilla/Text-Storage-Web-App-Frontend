...
test("Display error alert when the user cannot be authenticated.", () => {

    const { getByTestId } = render(<Login />);

    const response = { status: 401 };
    axios.get.mockRejectedValueOnce(new Error({ response: { status: 401 } }));
    ...
    let errorAlert = getByTestId("login-error-alert");
    expect(errorAlert).toBeInTheDocument();
    expect(errorAlert.textContent).toBe("Incorrect username or password.")


    // TODO: Ensure call and message displayed.
});
